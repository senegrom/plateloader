...
function buildAlgoLib() {
  ...
  function optimize(weights, mode, plateMax, PLATES, BAR, startStack, monotonic, sided, options = {}) {
    ...
    // The interval search. Every prefix and contiguous block of admitting sets
    // gets one all-pairs cost table, solved by prefix depth from the leaves
    // toward the root so only two adjacent layers of tables are live. Whole
    // blocks and row prefixes are shared only for exactly equal sequences of
    // feasible sets; only compact reconstruction choices survive. Candidates
    // are examined in a fixed order (stop, then plates ascending, then run
    // start descending, strict improvement only). No candidate is discarded.
