...
Small sessions retain the plain all-pairs interval engine. Larger state spaces use the same recurrence with compact storage: exactly equal feasible-set words share block tables and row prefixes; open-ending cells remain separate. Pinned starting states have distinct feasibility identities. Reconstruction choices store relative run starts, so sharing never changes physical order or tie-breaking.

The compact engine solves prefix-depth layers from the leaves toward the root. Each dependency adds exactly one plate, so only two adjacent layers of cost tables are needed. Reusable slabs avoid whole-store copies and allocation peaks; compact choices remain available for reconstruction. This is a storage policy, not a reduced search, timeout, stock cap or approximate answer. The direct-API `compactTables` option can force either storage path for parity tests.
...
The dependency-free Node suite covers the exact optimiser, state handling, worker and service-worker behavior, and deterministic build boundaries. Independent exhaustive-oracle tests cover open and closed endings, both sidedness settings, pinned starts, invalid rows, repeated targets and all three objectives. Complete-result parity checks compare both storage paths and the cooperative target. Subprocess regressions cover both identical repeats and 50 alternating high-stock sets, including process-memory bounds. Deadline, cancellation and concurrent-input tests cover the fallback.
