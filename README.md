...
The synchronous `optimize()` API and the worker retain their return-value contract. `optimizeAsync()` accepts the same eight positional arguments and ninth options argument, and resolves to the same complete results. Both entry points drive a single search in the same candidate order. Options include `{ leaveLoaded, timeLimitMs, signal }`; the async API also accepts `sliceMs` (clamped to 1–16 ms). There is no total deadline by default for direct API callers. Async inputs are copied before yielding; cancellation rejects with `AbortError` and timeout with `TimeoutError`. It uses `scheduler.yield()` when available and a timer task otherwise, not a microtask-only loop.
