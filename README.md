...
Long worker calculations expose Cancel and can be replaced by fresh input without accepting stale replies. A worker announces when computation begins; a runtime failure is reported rather than rerunning an expensive workload on the main thread. Browsers where workers cannot start use a resumable exact fallback with a 3 s total time budget and short execution slices (8 ms by default, checked periodically). Feasibility enumeration and the interval search both yield to real browser tasks; Cancel and fresh input abort the suspended calculation. Individual allocations and garbage collection may take longer than a slice. Expiry aborts with an explicit message and returns no partial or approximate solution.

The synchronous `optimize()` API and the worker retain their return-value contract. `optimizeAsync()` accepts the same eight positional arguments and ninth options argument, and resolves to the same complete results. Both entry points drive a single search in the same candidate order. Options include `{ leaveLoaded, timeLimitMs, signal }`; the async API also accepts `sliceMs` (clamped to 1–16 ms). There is no total deadline by default for direct API callers. Async inputs are copied before yielding; cancellation rejects with `AbortError` and timeout with `TimeoutError`. It yields through timer tasks, not a microtask-only loop or priority-boosted continuations that can starve other timers.
...
The dependency-free Node suite covers the exact optimiser, state handling, worker and service-worker behavior, and deterministic build boundaries. Independent exhaustive-oracle tests cover open and closed endings, both sidedness settings, pinned starts, invalid rows, repeated targets and all three objectives. A subprocess regression budget protects the 50-repeat high-stock case. Resumable-search tests also exercise deterministic intermediate pauses, deadlines reached during both search phases, cancellation, input snapshots and concurrent calculations.

The Playwright suite exercises the built app under the exact `/plateloader/` project path in desktop Chromium, mobile Chromium and mobile WebKit. It covers keyboard navigation, optional custom inventory, shared-state restoration, offline operation, ordered compact loading, scrollable wide stacks, workout progress, replanning, Undo, cancellation and stale worker replies. Follow-up regressions cover Undo from a nonzero workout position, preference migration/reloads, unavailable storage, fallback responsiveness and late fallback replies. Unit tests run on both Ubuntu and Windows in CI. Mobile emulation does not replace physical iPhone installation, update and touch testing; the WebKit clipboard-feedback test uses a stub because the permission override is not available there.
