...
Every valid set shows the physical plate order **from the collar outward**; the diagram marks carried plates and newly added ones. Compact view replaces the diagram and the denomination totals with the ordered stack, so every required plate stays listed; one line above the cards states the side and collar-outward direction instead of labelling every card. Denomination totals are a secondary inventory summary, not a suggested loading order.
